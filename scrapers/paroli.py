...
_ww_game_declaration = re.compile(r"var ww_game\s*=\s*")


def _extract_ww_game(script: str) -> dict[str, Any]:
	start = _ww_game_declaration.search(script)
	if not start:
		raise Exception("Could not find var ww_game declaration.")
...
def _coerce_ww_game(ww_game: dict[str, Any]) -> ParoliGame:
...
def fetch_and_parse() -> ParoliGame:
	...
	ww_game_object = _extract_ww_game(script)
