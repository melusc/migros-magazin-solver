...
def _extract_script_url(page: str) -> str:
	...
	if not match:
		raise SudokuException("Could not extract script url from sudoku page.")
...
def _coerce_sudoku(ww_game: Any) -> Sudoku:
	if not isinstance(ww_game, dict):
		raise SudokuException(f'Expected "ww_game" to be a dict, got {type(ww_game)}.')

	if "dimx" not in ww_game:
		raise SudokuException('Missing key "dimx" in ww_game.')

	dim_x_raw = ww_game["dimx"]
	if not isinstance(dim_x_raw, str):
		raise SudokuException(f"Expected dimx to be a string, got {type(dim_x_raw)}.")

	try:
		dim_x = int(dim_x_raw)
	except ValueError:
		raise SudokuException(
			f"Expected dimx to be a string-encoded integer, got {dim_x_raw}."
		)

	if "dimy" not in ww_game:
		raise SudokuException('Missing key "dimy" in ww_game.')

	dim_y_raw = ww_game["dimy"]
	if not isinstance(dim_y_raw, str):
		raise SudokuException(f"Expected dimy to be a string, got {type(dim_y_raw)}.")

	try:
		dim_y = int(dim_y_raw)
	except ValueError:
		raise SudokuException(
			f"Expected dimy to be a string-encoded integer, got {dim_y_raw}."
		)

	if "formmatrix" not in ww_game:
		raise SudokuException('Missing key "formmatrix" in ww_game.')

	form_matrix = ww_game["formmatrix"]
	if not isinstance(form_matrix, list):
		raise SudokuException(f"Expected formmatrix to be a list, got {type(form_matrix)}.")
	...
	for i, row in enumerate(form_matrix):
		if not isinstance(row, str):
			raise SudokuException(
				f"Expected formmatrix[{i}] to be a string, got {type(row)}."
			)
	...
	if "matrix" not in ww_game:
		raise SudokuException('Missing key "matrix" in ww_game.')

	matrix = ww_game["matrix"]
	if not isinstance(matrix, list):
		raise SudokuException(f"Expected matrix to be a list, got {type(matrix)}.")

	if len(matrix) != dim_y:
		raise SudokuException(
			f"Expected matrix to have {dim_y} rows, got {len(matrix)} rows."
		)

	if "show_Winfields" not in ww_game:
		raise SudokuException('Missing key "show_Winfields" in ww_game.')
	...
	for i, item in enumerate(win_fields_raw):
		if not isinstance(item, dict):
			raise SudokuException(
				f"Expected show_Winfields[{i}] to be a dict, got {type(item)}."
			)
	...
	for y, row in enumerate(matrix):
		if not isinstance(row, str):
			raise SudokuException(f"Expected matrix[{i}] to be a string, got {type(row)}.")
...
__all__ = ("SudokuBorder", "SudokuException", "fetch_and_parse_sudoku")
